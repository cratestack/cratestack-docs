...
```toml
[dependencies]
cratestack = { package = "cratestack-api", version = "0.8" }
```
...
```toml
cratestack = { package = "cratestack-pg", version = "0.8", default-features = false }
```
...
The `()` in third position is the **computed-field resolver**, and `()` is the right value
whenever the schema declares no `@computed` fields. A `db = None` schema can still declare
them — on a `type` returned by a procedure — in which case pass your resolver here instead;
see [Computed Fields](./computed-fields).
