...
```text
error: model `Widget` is not allowed in a schema with `datasource { provider = "none" }`
  no-database schemas own no data — remove the model, or switch the
  datasource to `provider = "postgresql"` / `provider = "sqlite"`
```
