...
29. **`transport grpc`** — a third mutually-exclusive generation style alongside REST and RPC: a real gRPC server runtime (CRUD-only surface, matching REST/RPC today) plus generated clients in all three languages — native Rust via `include_client_schema!` (`tonic`-based), a hand-rolled gRPC-Web TypeScript client, and a native Dart client. `cratestack generate-proto` emits the `.proto` contract standalone even without the `grpc` Cargo feature enabled
30. **composite primary keys** — `@@id([...])` / `@@unique([...])` on a model, with parser validation and Postgres/SQLite DDL emission; query builders, generated clients, and policy evaluation for composite keys are still catching up (see [Still Narrow Or Deferred](#still-narrow-or-deferred))
...
12. composite primary keys (`@@id([...])`) have parser validation and DDL emission, but query builders, generated clients, and policy evaluation don't yet have first-class composite-key support
13. `transport grpc` is CRUD-only — no procedures, no streaming, matching the current REST/RPC server surface; a Dart gRPC client generator's own middleware/link chain doesn't exist yet either (see item 25 above)
