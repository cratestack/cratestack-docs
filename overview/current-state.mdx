...
19. Decimal scalar with two implemented, additive backends (0.8.0+, cratestack#505) — `decimal-rust-decimal` (default) and `decimal-bigdecimal` can both be enabled at once, each dependent in the build graph choosing its own; a schema with a `Decimal` field requires a `decimal = RustDecimal | BigDecimal` argument on its `include_*_schema!` call, see [Scalars](../reference/scalars#the-decimal-macro-argument)
