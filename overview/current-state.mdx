...
11. multi-DB server support — `include_server_schema!` accepts a `db = Postgres` argument (or `db = None`, see above) but only Postgres is wired for an actual database backend today; MySQL and SQLite-via-sqlx are non-breaking future additions
12. `transport grpc` *procedures* — the generated gRPC service currently covers model CRUD only; procedures aren't wired in yet
13. `@@id([...])` composite primary keys are authorable and migratable but not yet loadable by a running server or embedded app — see item 30 above
