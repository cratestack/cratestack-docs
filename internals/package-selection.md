...
> **Staleness notice:** this document's crate/package inventory was last substantially updated around v0.3.0 (2026-05-12) and is now significantly behind current source (v0.7.15) — roughly 3 months and ~40 point-releases. Entire crates and packages shipped since then (the `cratestack-pg` / `cratestack-api` / `cratestack-sqlite` facade split, `cratestack-sql`, the client codegen and Studio crates, and most of the `packages/*` TypeScript split) are not reflected below. For the current, authoritative package list, run `ls crates/` and `ls packages/` in the source repo, or consult a live reference doc rather than this historical log.
...
**Updated 0.4.0+** — the public `cratestack` crate split into disjoint facades, selected via Cargo's `package =` rename: `cratestack = { package = "cratestack-pg" }` (Postgres + Axum + Rust client runtime), `cratestack = { package = "cratestack-sqlite" }` (rusqlite, native + wasm), and a third, `cratestack = { package = "cratestack-api" }`, added later for `db = None` procedures-only servers — `sqlx`/`cratestack-sqlx` are structurally absent from its dependency graph, not just feature-gated off. `cratestack-pg` itself later gained a default-on `postgres` Cargo feature gating its own `cratestack-sqlx` dependency, so a `db = None` consumer of `cratestack-pg` can shed sqlx with `default-features = false` instead of switching facades outright. `cratestack-pg` also gained a `crypto-aws-lc-rs` feature that is currently a reserved name only — enabling it is a hard `compile_error!`, not a working FIPS mode, since making it real needs the TLS backend to become a genuine choice across `cratestack-sqlx`/`cratestack-client-rust` first (both currently hard-select the `ring` rustls backend).

## Date

2026-04-26 (initial). 0.3.0 update: 2026-05-12. 0.4.0+ update: 2026-08-03.
...
Original recommended workspace (0.1.0 bootstrap — kept for history; see the real current layout below):
...
3. Dart client generation forked into two paths that both render through repo-managed MiniJinja templates (callers can override with a template directory): the `cratestack-client-flutter` FFI bridge crate (Dart → bridge JSON bytes → Rust → HTTP, transport codec ownership stays in Rust) is one path, but the actual default/`riverpod`-preset generated Dart talks to the server **directly via `dio`**, no FFI bridge in the loop — see [Client Runtime Architecture](../architecture/client-runtime) for the two paths in full. Both emit multiple sibling files per client package and expose canonical `fields` / `include` / `includeFields[path]` query params plus generated field/include constants, generated selection builders, and projected wrapper objects, even though exact selection-aware typed responses are still incomplete; the Rust side also exposes nested selection builders, a generated schema-native client facade, additive selected `get/list` helpers, and request-authorizer hooks over the canonical HTTP contract
